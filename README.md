...
- **Day 2**：嘗試爬蟲方案——以網路爬蟲從台灣手語辭典下載 3,495 個詞彙的示範影片，提取 MediaPipe 關鍵點後訓練 BiLSTM 分類器。然而遭遇三個根本性問題：
  1. **畫質問題**：辭典影片解析度低，MediaPipe 在靜態模式下僅能偵測到 353 筆有效幀（VIDEO 串流模式雖提升至 160,157 筆，但仍無法解決後續問題）
  2. **類別過多**：3,495 個詞彙、平均每類不到 2 個訓練序列，模型嚴重過擬合，訓練集 100% 但驗證集僅 37%
  3. **Domain Gap**：辭典影片為專業手語老師在攝影棚拍攝，與自己面對筆電攝影機的動作分布差異極大——即時辨識時信心度僅 6–15%，模型完全無法實用

  確認爬蟲方案行不通後，決定改為自行錄製訓練資料。

- **Day 3**：在 Claude Code 建議下，以**多角度、多速度、多位置**的方式自錄 32 個詞彙（每詞 15 次），讓模型學到手勢本身而非特定錄影環境。同步設計 179 維特徵（加入手臂方向、手臉距離、身體朝向等），解決過擬合與信心度不穩問題，最終達到 95.31% 驗證準確率，並完成 Web 部署。
